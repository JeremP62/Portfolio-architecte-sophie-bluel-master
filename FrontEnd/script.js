...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const validateBtn    = formAddPhoto ? formAddPhoto.querySelector('.btn-validate') : null;

  // Zone d’upload
  const photoUploadDiv = document.querySelector('.photo-upload');
  const photoLabel     = photoUploadDiv ? photoUploadDiv.querySelector('.photo-label') : null;
  const photoHint      = photoUploadDiv ? photoUploadDiv.querySelector('p') : null;
  let previewImg       = photoUploadDiv ? photoUploadDiv.querySelector('img.preview') : null;
...
const defaultOption = document.createElement('option');
  ...
  function isFormValid() {
    ...
    const catOk   = photoCategory && photoCategory.value !== '' && photoCategory.value !== null;
  ...
  if (photoCategory) photoCategory.addEventListener('change', updateValidateState);
